	...
	if(isset($_['module']) && $_['module']=='camera'){
		...
		<div class="row">
			...
			<div class="span12">
				...
				<div class="span5">
					<h5>Photo PI</h5>
					<button class="btn" id="camera_btn" onclick="javascript:ajax_snapshot()">Prendre une photo</button><br/>
					<img class="img-polaroid img-rounded" id="cameraPI" src="plugins/camera/view.jpg<?php echo '?'.time(); ?>" onError="this.onerror=null;this.src='plugins/camera/img/noimage.jpg';" > <br/>
        ...
        <!--<video controls="controls" width="200" height="200" autoplay="autoplay" >
      	...
      	<div class="span6">
      		...
      		Et enfin<br/>
      		<code>
      			sudo chown -R www-data:www-data <? echo $absolute_path ?>/plugins
      		</code><br/>
...
function camera_action_camera(){
	...
	switch($_['action']){
		...
		exec('raspistill -t 1 -w 400 -h 400 -o '.$absolute_path.'view.jpg',$output,$err);
		switch ($err){
			case 127:
			echo "Impossible de trouver raspistill, faites une mise à jour du Raspberry Pi";
			break;

			case 126:
			echo "YANA n'a pas les permissions pour accéder à la caméra";
			break;
