					...
					<p>Ce module vous permet de créer un évenement en fonction d'une date que le client (yana windows ou yana for android) 
						...
						<span class="row">
							<span class="span2">
								...
								<select class="input-medium" name="eventState" id="eventState">
									<option <?php if ($currentEvent->getState()) echo 'selected="selected"'; ?> value="1">Activé</option>
									<option <?php if (!$currentEvent->getState()) echo 'selected="selected"'; ?> value="0">Déactivé</option>
								</select>
							</span>
						</span>
				...
				<table class="table table-striped table-bordered table-hover">
					...
					foreach($events as $event){ 
						...
						<tr>
							...
							<td><?php if ($event->getState()) {echo "Activé";} else {echo "Déactivé";}?></td>
							<td>
								<a class="btn btn-warning" href="index.php?module=eventmanager&id=<?php echo $event->getId(); ?>"><i class="icon-edit"></i></a>
								<a class="btn btn-danger" href="action.php?action=eventmanager_delete_event&id=<?php echo $event->getId(); ?>"><i class="icon-remove"></i></a></td>
							</tr>
							<?php } ?>
						</table>
