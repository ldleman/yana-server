...
function propise_action(){
	...
	switch($_['action']){
		...
		Action::write(
			...
			$datas = $dataManager->loadAll(array('sensor'=>$_['id']),'time DESC');
			...
			Action::write(
				...
				function($_,&$response){	
					...
					if(empty($parameters['sensor'])){
						...
						$content .= '<div class="propise_widget" data-id="'.$sensor->id.'"><ul class="propise_view">';

						$content .= '<li class="propise_light" data-type="light"><i class="fa fa-sun-o"></i> <span>'.propise_convert_light($datas[0]->light).'</span>%</li>';
								...
								setInterval(function(){
									...
									$(".propise_widget").each(function(i,element){
										$.action({
											...
											$(".propise_light span").html(r.light);
										});
									});
